...
class MockCarWashRepo:
    ...
    def add_car(self, license_plate, car_color, is_dirty, hrs_parked, Price) ->None:
        ...
        db.session.commit()
        car : CarModel = CarModel.query.filter(CarModel.license_plate==license_plate).first()
    ...
    def update_car(self, license_plate, car_color, is_dirty, hours_parked,  price):
        car = CarModel.query.filter(CarModel.license_plate==license_plate).first()
    ...
    def remove_car(license_plate) ->None:
        car = CarModel.query.filter(CarModel.license_plate==license_plate).delete()
        db.session.commit()
